...
//
    @Query("Select s from CommentEntity s where s.profile.id=:profileId")
    Page<CommentEntity> findByProfileId(@Param("profileId") Integer profileId, Pageable pageable);

    @Query("Select s from CommentEntity s where s.post.id=:postId")
    Page<CommentEntity> findByArticleId(@Param("postId") Integer postId, Pageable pageable);
//
